...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn<NIND2){
    if((addr = ip->addrs[NDIRECT+1]) == 0)
      ...
      //FIRST BLOCK OF NIND2
    a = indirect[addr];
    if((addr=a[bn/NINDIRECT])==0){
      a[bn/NINDIRECT]=addr=indalloc();
    }

    //SECOND BLOCK OF NIND2
    a = indirect[addr];
    if((addr=a[bn%NINDIRECT])==0){
  ...
  if(bn<NIND3){
    if((addr = ip->addrs[NDIRECT+2]) == 0)
      ip->addrs[NDIRECT+2] = addr = indalloc();

    //FIRST BLOCK OF NIND3
    a = indirect[addr];
    if((addr=a[bn/NIND2])==0){
      a[bn/NIND2]=addr=indalloc();
    }

    //SECOND BLOCK OF NIND3
    a = indirect[addr];
    if((addr=a[bn/(NINDIRECT)])==0){
      a[bn/(NINDIRECT)]=addr=indalloc();
    }

    //THIRD BLOCK OF NIND3
    a = indirect[addr];
    if((addr=a[bn%(NINDIRECT)])==0){
      a[bn%(NINDIRECT)]=addr=balloc(ip->dev);
    }
